...
**手裏剣はおりがみくらぶの折り図に忠実な9工程で実装済み。** ❶観音折り→❷半分→❸端を斜め45°
(2枚は鏡写し)→❹先端を折って稲妻形→❺朱を裏返して90°回転・藍を中央へ→❻十字に重ねる→
❼朱の先端を藍のポケットへ差し込む→❽全体裏返し→❾藍も差し込んで完成(向かい合う尖りが同色)。
白面を上に始め、観音折りで色が出る(折り図と同じ)。幾何は tools/solve-shuriken.mjs で厳密検証
(差し込み折り線が相手の中央部の縁と同一直線)。エンジン拡張: `FoldOp.spinZ`(面内回転、❺で
裏返し180°+90°回転+移動を1オペ化)。差し込みは押し込み回転の擬似表現(README「既知の制約」)。
多層平畳みの層順テクニック3点(残差角の階段・軸上の紙はしをmovingから除外・折り目縁の頂点複製
+z補正)は README「2枚組みの表現」と shuriken.ts のコメント参照。差し込み方向 direction は
軸の向きで互い違い(❼: +1/-1、❾: -1/+1)、角度は❼8°/❾6°(相手の層内に収まる実測値)。
