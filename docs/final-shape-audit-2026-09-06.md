...
> 追記: コップの開口で側面が離れる欠陥を修正しました。[最新の接続検証](cup-opening-2026-09-06.md)。
> 旧判定は面の辺長のみを検証しており、隣り合う面の離れを見逃していました。

> この記録の工程数は、折り目づけ省略前のものです。その後ユーザー指定により
> 折り目づけだけの往復工程を省略しました。[現行工程数と検証](crease-preparation-omission.md)。
> 完成形と残した工程の形状・表裏、および以下の既知の課題は変更していません。
