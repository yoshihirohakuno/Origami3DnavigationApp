...
// 折り図(easy/animal-face/cat)の実測値。耳の折り線はあご先(原点)から
// (∓EX,±EY) へ。紙のふち上なので EX+EY=1。この角度で折ると耳先が
// (∓0.29,0.96) に立ち、完成形の幅:高さ≒1.2:1(折り図の実測 1.21)になる。
// 旧版は (∓0.68,±0.32) で耳が外へ寝ており、幅:高さ≒1.78:1 の横長だった
const EX = 0.575;
const EY = 0.425;
// てっぺんの折り線 y=TY(折り図の実測 0.785)。ふちとの交点が (∓TX,TY)
const TY = 0.785;
const TX = 1 - TY;
...
const V: [number, number][] = [
  ...
  [-1, 0], //  3: 左の角(左耳・上層)
  [-EX, -EY], //  4: 左耳折り線・外側(左下辺上・下層シート側)
  [-EX, EY], //  5: 左耳折り線の上層ミラー(左上辺上)
  [EX, -EY], //  6: 右耳折り線・外側(右下辺上・下層シート側)
  [EX, EY], //  7: 右耳折り線の上層ミラー(右上辺上)
  [-TX, TY], //  8: てっぺん折り線・左(左上辺上)
  [TX, TY], //  9: てっぺん折り線・右(右上辺上)
  [-TX, -TY], // 10: てっぺん折り線の下層ミラー・左(左下辺上・下層シート側)
  [TX, -TY], // 11: てっぺん折り線の下層ミラー・右(右下辺上・下層シート側)
  // ❷の縦折り目(x=0)が辺・折り線と交わる点。
  // 12=中心はあご先であり、両耳の折り線の起点でもある。❺の回転軸は [12,13]
  [0, 0], // 12: 中心(❶の折り線上・あご先)
  [0, TY], // 13: てっぺん折り線の中点(上層)
  [0, -TY], // 14: てっぺん折り線の中点(下層ミラー)
  // 下層側の複製(折り返しで手前に出る側)
  [-1, 0], // 15: 3 の下層複製(左耳の先)
  [1, 0], // 16: 1 の下層複製(右耳の先)
  [-EX, -EY], // 17: 4 の複製(左耳フラップの付け根)
  [EX, -EY], // 18: 6 の複製(右耳フラップの付け根)
  [-TX, -TY], // 19: 10 の複製(てっぺんフラップの付け根・左)
  [TX, -TY], // 20: 11 の複製(てっぺんフラップの付け根・右)
  // 21: 14 のフラップ側の複製。てっぺんフラップの面は❷の折り目(x=0)をまたぐので
  // ここで割らないと、左だけが回って面がねじれる(2026-08-15 追加)。
  // z 補正を中央の面と分けるため、14 を共有せず複製で持つ
  [0, -TY], // 21: てっぺんフラップの付け根・中(x=0)
];
...
const F: number[][] = [
  ...
  [12, 4, 10, 14], // 中央・左
  [14, 11, 6, 12], // 中央・右
  [2, 21, 19], // てっぺんフラップ・左(❷の折り目 x=0 で分割)
  [2, 20, 21], // てっぺんフラップ・右
  [15, 17, 12], // 左耳フラップ
  [16, 12, 18], // 右耳フラップ
];
...
const steps: FoldStep[] = [
  {
    ...
    folds: [
      {
        axis: [3, 1],
        moving: [2, 4, 6, 10, 11, 14, 17, 18, 19, 20, 21],
        type: 'mountain',
        angle: 177,
      },
    ],
...
export const catModel = withRigidFolds(source);
