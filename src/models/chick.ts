 ...
 *
 * 色: 紙の表は黄、裏は白で扱い、折り図に合わせて白い裏面を上にして始める。
 *
...
const T1X = -0.14;
const T2X = 0.24;
/** ❺の折り線が❶の折り線(x=0)と交わる y。13-14 を x=0 で内分した点 */
const TMY = 1 + T1X + (T1X / (T2X - T1X)) * (T1X + T2X); // 0.8232
...
const V: [number, number][] = [
  ...
  // ❶の折り線 x=0 と❺の折り線の交点。体の面をここで割らないと、
  // 折り目をつける工程(❶)で左半分だけが回って面がねじれる(2026-08-15 追加)
  [0, TMY], // 15: 上の層(13-14 の上)
  [0, -TMY], // 16: 下の層(11-12 の上)
];

// 体と先の面は❶の折り線 x=0 でも割る(15/16 と中心 3 が割り目の頂点)。
// 割らないと❶で左半分だけが回り、面がまたいだまま引き伸ばされる
const F: number[][] = [
  // 上半分(❹で手前へ回る=完成形で手前。裏の黄が見える)
  [2, 4, 5], // くちばしの外側(❸で折り返す部分)
  [5, 4, 6, 7], // プリーツの中(❷と❸の折り線のあいだ)
  [13, 0, 15], // 体の下の先・左(❺でうしろへ)
  [15, 0, 14], // 体の下の先・右
  [7, 6, 13, 15, 3], // 体・左
  [3, 15, 14, 1], // 体・右
  // 下半分(❹で動かない側)
...
// ❹で手前へ回る上半分(y>0)。15 も上半分なので忘れないこと
const UPPER = [0, 4, 6, 13, 14, 15];
...
export const chickModel = withRigidFolds(source);
