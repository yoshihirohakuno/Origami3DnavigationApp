 ...
 *
 * 折り線:
 * - **❷ は (-0.5,1)-(-1,-1)**。上のふちの「左の角と、まんなかの折り目のあいだの中点」から
 *   左下の角へ(傾き 1/4)。**上端は目印つきの厳密値**: 折り図❷のたての折り目は x=423.5px で
 *   正方形(323-524)の厳密な中心、左のふち 323 との中点は 373.25px、破線の重心は 372.7px
 *   (差 0.005単位)。折り図❸のフラップの左のふちも (-0.50,1)-(-0.99,-1)。
 *   この折りで**左上の角 (-1,1) が (-0.06,0.76) へ回る**(折り図❸の実測 (0.005,0.751))
 * - **❸ は y=0**(❶の折り目)。折り図❹の輪郭は左のふちが傾き 1/4 の台形で、
 *   上のふち 1.75 / 下のふち 2 の比(実測 178px/201px = 0.885、予測 0.875)と一致
 * - **❹ の折り線は折り後の (0,0)-(-1,-1)**(上のふちの中点と左下の角を結ぶ45°線。両端とも目印。
 *   折り図❹の破線の実測 (0.02,0)-(-1.02,-1)、上端は同パネルのたての折り目 x=408px と一致)。
 *   **これは1本の折りではなく、正方基本形と同じ袋つぶし(1軸回転2回の連鎖)**。
 *   1本にすると輪郭は同じでも層の行き先が変わり、鼻の下の白いくさびが出ない
 *   (折り図❺の白 12.8% に対して 2% になる)。連鎖にすると 11% で、
 *   白のくさびの左のふち(-0.602)が折り図の実測(-0.62)と一致する。
 *   **残る差**: 折り図はこの白を x=+0.16 まで描いているが、モデルでは x=0 で止まる
 *   (そこから右は❸で下りた上半分がおおうため)。1.8ポイントぶんの差
 *
