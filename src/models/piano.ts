...
const steps: FoldStep[] = [
  ...
  },
  {
    // ❺ 下を折り上げてけんばんに
    folds: [
      // **手前へ折り上げる(谷折り)**。折り図❺の矢印は下から手前へ回る谷折りで、
      // 折り返した帯は裏の白を見せる = 折り図❻のけんばんの白鍵。
      // 帯の左右のはしだけ灰色なのは、❷で内へ折った帯がここで前後入れ替わるため。
      // 11/15(帯のふち)は折り線の上にあるが、❷で持ち上がった z を持っている。
      // moving に入れると z が反転して正面へ出てしまうので、軸上の紙はしとして除外する
      { axis: [12, 14], moving: [16, 17, 18, 19, 20], type: 'valley', angle: 174 },
    ],
    description: {
...
export const pianoModel = withRigidFolds(source);
