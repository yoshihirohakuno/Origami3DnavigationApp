...
test('the shorts close to a square and open a white gap between the legs', () => {
  const m=modelOf('shorts'),p=computeFoldState(m,m.steps.length).positions;
  const used=[...new Set(m.faces.flat())];
  const xs=used.map(vi=>p[vi].x), ys=used.map(vi=>p[vi].y);
  // よこ2の紙の左右のふちを中心線へ折るので、たてよこ1の正方形のズボンになる
  assert.ok(Math.abs(Math.min(...xs)+.5)<1e-8 && Math.abs(Math.max(...xs)-.5)<1e-8);
  assert.ok(Math.abs(Math.min(...ys)+.5)<1e-8 && Math.abs(Math.max(...ys)-.5)<1e-8);
  // 中央の下だけ白いVが開く(両足のあいだ)。先は紙の中心の高さ
  for(const [x,y] of [[0,-.45],[0,-.1],[-.18,-.48],[.18,-.48]])
    assert.equal(visibleAt(m,x,y)?.front,false,`the leg gap stays white at ${x},${y}`);
...
test('the water bottle narrows its white cap over a colored body', () => {
  const m=modelOf('water-bottle'),p=computeFoldState(m,m.steps.length).positions;
  const used=[...new Set(m.faces.flat())];
  const xs=used.map(vi=>p[vi].x), ys=used.map(vi=>p[vi].y);
  // ❶で上を y=.5 へ、❸で左右を x=±.5 へ。高さ1.5・幅1になる
  assert.ok(Math.abs(Math.min(...xs)+.5)<1e-8 && Math.abs(Math.max(...xs)-.5)<1e-8);
  assert.ok(Math.abs(Math.min(...ys)+1)<1e-8 && Math.abs(Math.max(...ys)-.5)<1e-8);
  // ふたは白、胴は色の面。境目はよこの中心線
  for(const [x,y] of [[0,.4],[-.2,.2],[.2,.05]])
    assert.equal(visibleAt(m,x,y)?.front,false,`the cap stays white at ${x},${y}`);
  for(const [x,y] of [[0,-.2],[-.4,-.6],[.4,-.9]])
    assert.equal(visibleAt(m,x,y)?.front,true,`the body stays colored at ${x},${y}`);
  // ❹で上の角を落とすので、ふたは胴より細い
...
test('the shoe keeps a white opening above a colored body and slants one toe', () => {
  const m=modelOf('shoes'),p=computeFoldState(m,m.steps.length).positions;
  const used=[...new Set(m.faces.flat())];
  const xs=used.map(vi=>p[vi].x), ys=used.map(vi=>p[vi].y);
  // ❷で上を y=.84 で後ろへ、❹で下を y=-.16(=.84-1)で折り上げる。高さは1
  assert.ok(Math.abs(Math.max(...ys)-.84)<1e-8 && Math.abs(Math.min(...ys)+.16)<1e-8);
  // ❸で幅1.5。❺でうらがえすので、外形は x=-1..0.5
  assert.ok(Math.abs(Math.min(...xs)+1)<1e-8 && Math.abs(Math.max(...xs)-.5)<1e-8);
  // 上の帯だけが白い裏。下の胴は色の面
  for(const [x,y] of [[0,.8],[.3,.8],[-.2,.75]])
    assert.equal(visibleAt(m,x,y)?.front,false,`the opening stays white at ${x},${y}`);
  for(const [x,y] of [[0,.6],[-.9,.3],[-.95,0],[.45,.5]])
    assert.equal(visibleAt(m,x,y)?.front,true,`the body stays colored at ${x},${y}`);
  // ❹でつま先をななめに落とすので、うらがえすと左上が欠ける
  assert.equal(visibleAt(m,-.9,.8),undefined,'the toe corner is cut away');
  assert.equal(visibleAt(m,.6,.3),undefined,'nothing reaches past the folded edge');
  // ❷のあと(折り図❹)は、折り返した1枚の上のはしだけ白が残る
